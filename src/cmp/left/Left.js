...
const onClick = (e) => {
  ...
  if (e.key == '個人資料') {
    ...
    window.open(`/reserve`,'_self')
  }

  else if(e.key == '待評論'){
    window.open(`/tbcomment`,'_self')
  }
...
const items = [
  getItem('用戶資訊', '1', <PieChartOutlined />, [
    getItem("個人資料", '個人資料', <PieChartOutlined />),
    getItem("即將入住",'即將入住',<PieChartOutlined />),
    getItem("預定中",'預定中',<PieChartOutlined />),
    getItem("待評論",'待評論',<PieChartOutlined />),
  ]),
